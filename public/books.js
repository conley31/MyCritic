...
var populateHtml = function(){
    ...
    for(var i = 0; i < 20; i++){
        if(bookObj[i]["GoodreadsResponse"] != undefined){
            ...
            html += "<div id=\"" + i + "\" style=\"margin-left: 10%; border-bottom-style: solid; border-width: 2px\"; onclick=\"window.location=\'/bookInfo/" + bookObj[i]["GoodreadsResponse"]["book"]["id"]["_text"] + "\'\"'><h1>" + bookObj[i]["GoodreadsResponse"]["book"]["title"]["_cdata"] + "</h1></div>";
            }
            else{
                html += "<div id=\"" + i + "\" style=\"margin-left: 10%; border-bottom-style: solid; border-width: 2px\"; onclick=\"window.location=\'/bookInfo/" + bookObj[i]["GoodreadsResponse"]["book"]["id"]["_text"] + "\'\"'><h1>" + bookObj[i]["GoodreadsResponse"]["book"]["title"]["_text"] + "</h1></div>";			  			
